...
def test_forward_secrecy_long_term_key_compromise():
    ...
    """
    alice_secret1 = generate_key()
    bob_secret1 = generate_key()
    counters1 = {"bob": 0}
    # Alice and Bob exchange messages with session keys
    msg1 = build_encrypted_message("alice", "hello bob", {"bob": alice_secret1}, counters1)
    # Session keys are rotated (ephemeral)
    alice_secret2 = generate_key()
    bob_secret2 = generate_key()
    counters2 = {"bob": 0}
    msg2 = build_encrypted_message("alice", "new session", {"bob": alice_secret2}, counters2)
    # Attacker compromises bob_secret2 (current session)
    # Attacker should NOT be able to decrypt msg1 (old session)
    with pytest.raises(InvalidTag):
        decrypt_incoming_message(msg1, "bob", {"alice": bob_secret2})
    # Attacker CAN decrypt msg2 (current session)
    assert decrypt_incoming_message(msg2, "bob", {"alice": bob_secret2}) == "new session"
...
def test_forward_secrecy_session_key_compromise():
    ...
    bob_secret1 = generate_key()
    counters1 = {"bob": 0}
    msg1 = build_encrypted_message("alice", "session1", {"bob": alice_secret1}, counters1)
    # Attacker gets bob_secret1
    assert decrypt_incoming_message(msg1, "bob", {"alice": bob_secret1}) == "session1"
    # Session keys rotate (new shared secret)
    alice_secret2 = generate_key()
    bob_secret2 = generate_key()
    counters2 = {"bob": 0}
    msg2 = build_encrypted_message("alice", "session2", {"bob": alice_secret2}, counters2)
    # Attacker CANNOT decrypt future messages
    with pytest.raises(InvalidTag):
        decrypt_incoming_message(msg2, "bob", {"alice": bob_secret1})
...
def test_forward_secrecy_replay_attack():
    ...
    bob_secret1 = generate_key()
    counters1 = {"bob": 0}
    msg1 = build_encrypted_message("alice", "replay test", {"bob": alice_secret1}, counters1)
    # Session keys rotate
    alice_secret2 = generate_key()
    bob_secret2 = generate_key()
...
def test_forward_secrecy_tampering_attack():
    ...
    """
    alice_secret = generate_key()
    bob_secret = generate_key()
    counters = {"bob": 0}
    msg = build_encrypted_message("alice", "tamper test", {"bob": alice_secret}, counters)
    ...
    with pytest.raises(InvalidTag):
        decrypt_incoming_message(tampered, "bob", {"alice": bob_secret})
